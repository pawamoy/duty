...
def _find_ruff() -> str:
    ...
    try:
        ...
        for path in paths.split(os.pathsep):
            ruff = os.path.join(path, "ruff")  # noqa: PTH118
            if os.path.exists(ruff):  # noqa: PTH110
                return ruff
        py_version = f"{sys.version_info[0]}.{sys.version_info[1]}"
        pypackages_bin = os.path.join("__pypackages__", py_version, "bin")  # noqa: PTH118
        ruff = os.path.join(pypackages_bin, "ruff")  # noqa: PTH118
        if os.path.exists(ruff):  # noqa: PTH110
            return ruff
    return "ruff"
