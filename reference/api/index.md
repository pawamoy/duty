...
- **`__call__`** – Run the command.
- **`add_args`** – Append CLI arguments.
- **`check`** – Run ty on the given files or directories.
...
Source code in `src/duty/_internal/tools/_ty.py`
...
- **`paths`** (`str`, default: `()` ) – Files or directories to type check.
- **`add_ignore`** (`bool | None`, default: `None` ) – Adds ty: ignore comments to suppress all rule diagnostics.
- **`project`** (`str | None`, default: `None` ) – Run the command within the given project directory.
- **`python`** (`str | None`, default: `None` ) – Path to your project's Python environment or interpreter.
- **`typeshed`** (`str | None`, default: `None` ) – Custom directory to use for stdlib typeshed stubs.
- **`extra_search_path`** (`list[str] | None`, default: `None` ) – Additional path to use as a module-resolution source (can be passed multiple times).
- **`python_version`** (`str | None`, default: `None` ) – Python version to assume when resolving types (possible values: 3.7, 3.8, 3.9, 3.10, 3.11, 3.12, 3.13, 3.14).
- **`python_platform`** (`str | None`, default: `None` ) – Target platform to assume when resolving types.
- **`verbose`** (`bool`, default: `False` ) – Use verbose output (or -vv and -vvv for more verbose output).
- **`quiet`** (`bool`, default: `False` ) – Use quiet output (or -qq for silent output).
- **`config`** (`list[str] | None`, default: `None` ) – A TOML <KEY> = <VALUE> pair overriding a specific configuration option.
- **`config_file`** (`str | None`, default: `None` ) – The path to a ty.toml file to use for configuration.
- **`output_format`** (`str | None`, default: `None` ) – The format to use for printing diagnostic messages (possible values: full, concise, gitlab, github).
- **`error_on_warning`** (`bool | None`, default: `None` ) – Use exit code 1 if there are any warning-level diagnostics.
- **`exit_zero`** (`bool | None`, default: `None` ) – Always use exit code 0, even when there are error-level diagnostics.
- **`watch`** (`bool | None`, default: `None` ) – Watch files for changes and recheck files related to the changed files.
- **`error`** (`list[str] | None`, default: `None` ) – Treat the given rule as having severity 'error'. Can be specified multiple times.
- **`warn`** (`list[str] | None`, default: `None` ) – Treat the given rule as having severity 'warn'. Can be specified multiple times.
- **`ignore`** (`list[str] | None`, default: `None` ) – Disables the rule. Can be specified multiple times.
- **`respect_ignore_files`** (`bool | None`, default: `None` ) – Respect file exclusions via .gitignore and other standard ignore files.
- **`force_exclude`** (`bool | None`, default: `None` ) – Enforce exclusions, even for paths passed to ty directly on the command-line.
- **`exclude`** (`list[str] | None`, default: `None` ) – Glob patterns for files to exclude from type checking.
- **`no_progress`** (`bool | None`, default: `None` ) – Hide all progress outputs.
- **`color`** (`str | None`, default: `None` ) – Control when colored output is used (possible values: auto, always, never).
